...
async def run_pipeline(repo_name: str, pr_number: int):
    """
    The main pipeline. Runs all three agents in sequence.
    Saves the result to the database after completion.
    ...
    for attempt in range(1, MAX_RETRIES + 2):
        try:
            ...
            # ── Stage 3: Post ───────────────────────────────
            logger.info("[Orchestrator] Stage 3 — Posting review to GitHub")
            poster = PosterAgent()
            ...
            logger.info("[Orchestrator] Stage 4 — Saving to database")
            db = SessionLocal()
            try:
                saved = await asyncio.to_thread(
                    save_review, db, context, result, success
                )
                logger.info(
                    f"[Orchestrator] Stage 4 complete — "
                    f"Saved with ID: {saved.id}"
                )
            finally:
                db.close()
